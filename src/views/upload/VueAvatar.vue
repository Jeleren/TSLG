...
<script>
  export default {
    ...
    methods: {
      ...
      getDimensions () {
        let client_width=document.documentElement.clientWidth*307/1920,client_height=document.documentElement.clientWidth*231.75/1920
        return {
          width: client_width,
      ...
      loadImage (imageURL) {
        ...
        imageObj.onload = () => {
          ...
          if(imageObj.width<396||imageObj.height<296){
            self.$emit('setStatus',false)
          }else{
            self.$emit('setStatus',true)
          }
